...
11. **Upstream bugs stay in our own tracker (maintainer decision, #35):** During v0.1 we do **not** report anything to an upstream or third-party tracker (Codeberg DAWO-Core, mijn-bureau-infra, or any other). That is outside our scope and comes later, as a separate decision by the maintainer. Instead: (a) open our own GitHub issue with the label `upstream` (symptom, cause, suggested upstream fix); (b) record the problem and **our workaround** (or why none is needed) in the "Known upstream issues" table of `docs/deviations.md`, in the same PR that introduces or confirms the workaround; (c) keep that row current when the upstream pin moves. Do not ask again whether to report upstream; this is decided. It applies to all agents equally.
...
- **KVM merge gate (maintainer decision, 2026-09-26):** a PR that changes what a KVM boot test exercises (host modules, guest/cloud-init, `vm/`, flake test definitions) is merged only after a **green KVM run** of the affected tests (`test-appliance-boot`, `test-guest-boot`, `test-install-e2e`, `test-installer-boot`) has been posted on the PR — by the KVM-capable session, or by any session whose machine can run those tests itself. Label such PRs `needs-kvm`. Docs-only and offline-test-only PRs are not affected.
