...
| R26 | **The Ubuntu guest really boots** (nested KVM): pinned cloud image (fetched by its manifest SHA-256), fixed lease 192.168.150.10, SSH with the host-generated key, hostname `mb.dawo.internal`, cloud-init finished with only the two offline-caused module failures (apt `qemu-guest-agent`, K3s `runcmd`), appliance CA in the guest trust store, and the CA cert+key transported to `/etc/dawo-appliance/{ca.crt,ca.key}` (root-only) for `apps/mijn-bureau/deploy.sh` (ADR 0004, issue #6) | `test-guest-boot` (needs nested KVM; ~5 min; 2026-09-26: SSH at 168 s, cloud-init at 287 s) | domain running, DHCP lease, `ssh ops@… hostname -f`, `cloud-init status --format json`, CA line in `/etc/ssl/certs/ca-certificates.crt`, `dawo-appliance-guest-status`, `ca.crt`/`ca.key` present in the guest with `ca.key` `600:root:root` — **added with this change, not yet run** (needs a KVM-capable machine) |
