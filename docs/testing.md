...
| R29 | **The live USB image boots a machine without touching its disk** (ADR 0006): the real `.#appliance-live-iso` under UEFI from an emulated USB stick; desktop, guest SSH and K3s node Ready reported by the image's own `DAWO-LIVE` report; the same report and the journal land on a `DAWO_LOGS` USB stick; a patterned 1 GiB internal disk is byte-for-byte unchanged | `test-live-iso-boot` (needs nested KVM; ~9 min incl. image build; 2026-09-26: desktop 19 s, guest SSH 92 s, K3s Ready 143 s) | serial `DAWO-LIVE: ... DONE ok`, `progress.txt`/`journal.txt` on the log stick (mtools), SHA-256 of the internal disk before/after |
