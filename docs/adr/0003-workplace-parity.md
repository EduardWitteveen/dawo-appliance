...
3. **Every deviation is deliberate and listed here.** Anything not on this list
   ...
   | **Guest operator SSH key** (`/var/lib/dawo-appliance/ssh/id_ed25519`, Slice 4a) is `0600`, owned by `dawo` (#75: an earlier `0640 root:libvirtd` made OpenSSH refuse the key for root), not root-only | Slice 7's health check (`health/README.md`) runs as `dawo` and must SSH into the guest to poll K3s/cert-manager; `dawo` already joins `libvirtd` (`hosts/appliance/virtualisation.nix`), so no new group was introduced. **Demo posture, not production**, exactly like the generated install password: never world-readable, never leaves the host except via cloud-init at install time (Slice 4a) | no (background service reads it) |
