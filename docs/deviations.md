...
| D21 | Private key files are root-only (0600/0700, owner and group `root`) | The guest operator SSH key and its directory are `0640`/`0750`, group `libvirtd` (`hosts/appliance/guest-vm.nix`) | Slice 7's health check runs as `dawo` (already in `libvirtd`, `hosts/appliance/virtualisation.nix`) and must read this key to SSH into the guest; never world-readable, never in Git | active | ADR 0003, #12 |
