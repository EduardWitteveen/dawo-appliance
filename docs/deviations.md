...
| D25 | DAWO pilot workplaces are installed on the device's disk | The default medium is a live USB (`nixosConfigurations.appliance-live`, `hosts/appliance/live.nix`): the same system, booted from the stick with a tmpfs root; the internal disk is never written. Installing stays available as an option | The audience tries it on a laptop whose only disk holds Windows; wiping it is not acceptable for a demo. Same workplace, different boot medium | active (first slice, no data partition yet) | ADR 0006, #93 |
...
| D26 | nixpkgs' libvirt encrypts its secrets key with `systemd-creds` in auto mode (TPM2 or a host key on persistent storage) | On the live USB only, the key is encrypted with the host key explicitly (`--with-key=host`) | Auto mode refuses on a live system (no usable TPM2, host key on tmpfs), so libvirtd and the guest never started; key and secrets are both in RAM and gone at power-off | active (live only) | `hosts/appliance/live.nix`, #93 |
