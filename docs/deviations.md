...
| D24 | All repository content is in English (`AGENTS.md` rule 2) | Documents written for the non-technical target audience are in Dutch instead: the `README.md` preamble, `docs/purpose.md`, `docs/demo-script.md`, `docs/demo-hardware.md`, and everything under `docs/audience/` (`docs/README.md` indexes the current set — check there, not here, for the exact file list) | Written for municipal civil servants, policy advisors and CISOs, who need Dutch-language material before the English technical documentation; all code, Nix modules, tests and architecture ADRs stay English-only per the rule | active | `AGENTS.md` rule 2 exception, #44 |
