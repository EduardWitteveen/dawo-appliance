...
| File | Role |
| --- | --- |
| `configuration.nix` | Aggregate: imports the four modules below, sets the hostname, ships the bootstrap. Defines nothing that shapes the user experience. |
...
| `appliance-ca.nix` | Addition (Slice 4b, ADR 0004): `dawo-appliance-ca.service` generates the per-install CA once at first boot (`/var/lib/dawo-appliance/ca/ca.crt` 0644, `ca.key` 0600 root, `ca.crt.sha256`; EC P-256, CN "DAWO appliance local CA", 10 years) before the display manager; trusts it in Firefox via the merged policy `Certificates.Install` and in Chromium via a per-login user service (`certutil` into `~/.pki/nssdb`); `/etc/dawo-appliance/ca.env` names the files for `curl --cacert`. Options `appliance.ca.*`. The private key on disk is a demo choice, not for production. Guest and cluster trust: Slices 4a/6. |
